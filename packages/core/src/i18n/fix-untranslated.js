...
const { parseJsonText } = require('./lib/stringUtils');
...
const autoDir = detectLocalesDir(projectRoot, srcRoot);
...
async function main() {
  ...
  const locales = await listLocales(autoDir, { includeJsonFiles: localeLayout === 'single' });
  if (!locales.includes(sourceLocale)) {
    console.error(`[fix-untranslated] No "${sourceLocale}" source locale found under ${autoDir}`);
    process.exit(1);
  }
  ...
  for (const locale of locales) {
    ...
    if (localeLayout === 'single') {
      const locFile = path.resolve(autoDir, `${locale}.json`);
      const enFile = path.resolve(autoDir, `${sourceLocale}.json`);
      if (!existsSync(enFile)) continue;

      const locJson = existsSync(locFile) ? await readJsonSafe(locFile) : {};
      const enJson = await readJsonSafe(enFile);
      if (!enJson || locJson === null) {
    ...
    const enRoot = path.resolve(autoDir, sourceLocale);
