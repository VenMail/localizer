...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  const handleSet = (lng: string) => {
    setOpen(false);
    setLocale(lng);
  };

  const badgeKey = t(`LanguageSwitcher.text.${locale}`);
  const badgeText = badgeKey === `LanguageSwitcher.text.${locale}` ? locale.toUpperCase() : badgeKey;
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()}>
          ...
          <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t("LanguageSwitcher.text.select_language")}
