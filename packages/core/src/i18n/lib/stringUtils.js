...
/**
 * Strip a leading UTF-8 BOM (U+FEFF) if present. JSON.parse does not tolerate
 * the BOM byte order mark, so every locale-file read path must normalize.
 */
...
/**
 * Safe JSON.parse that removes a leading UTF-8 BOM before delegating to
 * `JSON.parse`. Throws the same errors as JSON.parse on malformed input.
 */
