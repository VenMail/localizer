...
function isTranslatableText(text) {
  ...
  // Must have at least one letter (ASCII or Unicode — CJK/Cyrillic/etc.)
  let hasAnyLetter;
  try {
    hasAnyLetter = /\p{L}/u.test(trimmed);
  } catch {
    hasAnyLetter = /[a-z]/i.test(trimmed);
  }
  if (!hasAnyLetter) {
    return false;
  }

  // Non-ASCII letter scripts skip the English-specific heuristics below —
  // those were written for a-z and produce false negatives for CJK etc.
