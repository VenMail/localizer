...
function hasBasicRequirements(text) {
  ...
  // Must have at least one letter. Count Unicode letters so CJK
  // (Chinese/Japanese/Korean), Cyrillic, Greek, Hebrew, Arabic, etc.
  // qualify — not just ASCII a-z.
  let letterCount = 0;
  try {
    letterCount = (trimmed.match(/\p{L}/gu) || []).length;
  } catch {
    // Fallback for older runtimes without Unicode property escapes.
    letterCount = (trimmed.match(/[a-zA-Z]/g) || []).length;
  }
...
function isPlaceholderOnly(text) {
  ...
  // Accept any Unicode letter (CJK/Cyrillic/Greek/etc.), not just ASCII.
  let hasAnyLetter;
  try {
    hasAnyLetter = /\p{L}/u.test(stripped);
  } catch {
    hasAnyLetter = /[a-zA-Z]/.test(stripped);
  }
  if (!stripped || !hasAnyLetter) {
    return true;
  }
...
function looksLikeHumanText(text) {
  ...
  // Non-ASCII scripts (CJK, Cyrillic, Greek, Hebrew, Arabic, etc.) don't fit
  // the English heuristics below. If the text contains a non-ASCII letter,
  // treat it as human text — the upstream validators have already rejected
  // code/CSS/URL/technical content.
  try {
