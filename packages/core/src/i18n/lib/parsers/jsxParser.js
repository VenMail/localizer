...
class JsxParser extends BaseParser {
  ...
  parse(content, options = {}) {
    ...
    // Message-style call patterns (e.g. "message.error", "message.success").
    // Each entry is a dotted path; first string arg is treated as extractable.
    this.messagePatterns = Array.isArray(options.messagePatterns) ? options.messagePatterns : [];
    // Property-name allowlist for object-literal extraction in non-JSX configs.
    this.tsExtractKeys = Array.isArray(options.tsExtractKeys) ? options.tsExtractKeys : [];
  ...
  processObjectProperty(node, parent, results, shouldTranslate) {
    ...
    // Configurable allowlist (e.g. `name`, `placeholder`, `tooltip`) for
    // object-literal configs (animation tables, dropdown options, etc.).
    // Guard against picking up identifier-like values such as
    // `{ value: 'bounceIn' }`: reject single-token camelCase/snake_case
    // identifiers that clearly aren't user-facing text. Capitalised English
    // words ("Save", "OK") and any string containing a space or non-ASCII
    // letter still extract — `shouldTranslate` handles the final filter.
    if (!kind && this.tsExtractKeys && this.tsExtractKeys.includes(propName)) {
      if (this.isStringLiteral(valueNode)) {
        const valueText = (this.getStringValue(valueNode) || '').trim();
        if (valueText) {
          const hasNonAsciiLetter = /[^\x00-\x7F]/.test(valueText);
          const hasSpace = /\s/.test(valueText);
          const looksLikeIdent =
            !hasSpace && (
              /^[a-z][a-zA-Z0-9]*$/.test(valueText) ||        // camelCase
              /^[a-z][a-z0-9_]*$/.test(valueText) ||         // snake_case
              /^[a-z][a-z0-9-]*$/.test(valueText)            // kebab-case
            );
          if (hasNonAsciiLetter || hasSpace || !looksLikeIdent) {
            kind = propName === 'placeholder' ? 'placeholder'
              : propName === 'title' ? 'heading'
  ...
  processCallExpression(node, results, shouldTranslate) {
    ...
    // Configurable message-style calls (e.g. `message.error('...')`,
    // `notify.warning('...')`). Matched via dotted path exactly.
    if (calleeName && this.messagePatterns && this.messagePatterns.includes(calleeName)) {
      if (args.length > 0) {
        this.collectPatternsFromArg(args[0], 'toast', results, shouldTranslate);
      }
    }
