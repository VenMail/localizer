...
const projectRoot = process.env.AI_I18N_PROJECT_ROOT ? path.resolve(process.env.AI_I18N_PROJECT_ROOT) : path.resolve(__dirname, '..');
...
const outputDir = detectLocalesDir(projectRoot, srcRoot);
const sourceLocale = getSourceLocale(projectRoot);
const localeLayout = getLocaleLayout(projectRoot);
const messagePatterns = getMessagePatterns(projectRoot);
const tsExtractKeys = getTsExtractKeys(projectRoot);
...
(async () => {
  try {
    ...
    const groupedDir = path.resolve(outputDir, sourceLocale);
    ...
    if (existsSync(groupedDir)) {
      ...
      // Single-file layout: <outputDir>/<sourceLocale>.json
      const autoSingleFile = path.resolve(outputDir, `${sourceLocale}.json`);
      if (existsSync(autoSingleFile)) {
        ...
        // Older projects may only have a runtime base file at resources/js/i18n/<locale>.json
        const legacyRuntimeFile = path.resolve(projectRoot, 'resources', 'js', 'i18n', `${sourceLocale}.json`);
    ...
    const localeDir = path.resolve(outputDir, sourceLocale);
    if (localeLayout === 'grouped') {
    ...
    if (localeLayout === 'grouped' && existsSync(localeDir)) {
      try {
    ...
    let fileCount = 0;
    if (localeLayout === 'single') {
      // Flatten perRoot into a single top-level object keyed by namespace group.
      const merged = {};
      for (const tree of Object.values(perRoot)) {
        if (!tree || typeof tree !== 'object') continue;
        for (const [group, subtree] of Object.entries(tree)) {
          merged[group] = subtree;
        }
      }
      const outPath = path.resolve(outputDir, `${sourceLocale}.json`);
    ...
    const writtenRoot = localeLayout === 'single' ? outputDir : localeDir;
    console.log(`\n[i18n-extract] Wrote ${fileCount} ${localeLayout} file(s) under ${path.relative(projectRoot, writtenRoot)}`);
  } catch (error) {
